...
if(empty($_POST)) {
	$tpl->title = 'Eroor!';
	$tpl->msg = 'Please select one more item for payment';
...
if(!$sql->runQuery($query)) {
	$tpl->title = 'Error';
	$tpl->msg = 'Updating database fail, Please contact support team now.';
	$tpl->display(TPL_DIR . 'message.tpl.php');
	exit(0);
}
...
$mail_from = 'admin@keystores.net';
$mail_sub = 'Order confirm';
$siteURL = SITE_URL;
$mail_mesg = <<< HTML
Hello, Customer with name: {$_SESSION['username']},<br />
You have been order total: {$order['quantity']} items from www.keystores.net. See more detail your order.<br />
{$mailMsg}
--------------------------------------<br />
Detail your order:<br />
======================================<br />
Key Stores dot Net<br />
HTML;
...
$headers .= 'From: <'.$mail_from.'>' . "\r\n";
	...
	$tpl->title = 'Error';
	$tpl->msg = 'This processing fail. Please try again later';
	$tpl->display(TPL_DIR . 'message.tpl.php');
}
*/
	$tpl->title = 'Alert';
	$tpl->msg = <<< HTML
To see more detail your order from keystores.net. Please check your email from US, Or Please login follow link <a href="http://keystores.net/index.php?view=transaction">To see order status</a> Your items you had order.
HTML;
	$tpl->display(TPL_DIR . 'message.tpl.php');
?>